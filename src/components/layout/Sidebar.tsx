    ...
    <aside className="w-64 h-screen bg-sidebar border-r border-sidebar-border flex flex-col">
      ...
      <nav className="flex-1 py-6 px-3 space-y-1 overflow-y-auto custom-scrollbar">
        {navItems.map((item) => {
          ...
          return (
            <NavLink key={item.path} to={item.path} onClick={handleNavClick} className={cn('flex items-center gap-3 px-3 py-3 md:py-2.5 rounded-lg transition-all duration-200 group min-h-[48px] md:min-h-0', isActive ? 'bg-primary/15 text-primary border border-primary/20' : 'text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground')}>
      ...
      <div className="py-4 px-3 border-t border-sidebar-border space-y-1">
        {filteredBottomItems.map((item) => {
          const isActive = location.pathname === item.path;
          return (
            <NavLink key={item.path} to={item.path} onClick={handleNavClick} className={cn('flex items-center gap-3 px-3 py-3 md:py-2.5 rounded-lg transition-all duration-200 group min-h-[48px] md:min-h-0', isActive ? 'bg-primary/15 text-primary' : 'text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground')}>
      ...
      <div className="p-4 border-t border-sidebar-border">
        <div className="flex items-center gap-3 px-2">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary/50 to-accent/50 flex items-center justify-center"><span className="text-sm font-semibold text-foreground">GA</span></div>
          <div className="flex-1 min-w-0"><p className="text-sm font-medium text-foreground truncate">GovMesh Admin</p><p className="text-xs text-muted-foreground truncate">admin@govmesh.com.br</p></div>
        </div>
      </div>
    </aside>
  );
